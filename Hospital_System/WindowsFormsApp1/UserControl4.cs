...
namespace WindowsFormsApp1
{
    public partial class UserControl4 : UserControl
    {
        ...
        private void LoadPatientsFromFile()
        {
            try
            {
                if (!File.Exists(@"files\patients.txt")) return;

                string[] lines = File.ReadAllLines(@"files\patients.txt");
                count = 0;
        ...
        private void SavePatientsToFile()
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(@"files\patients.txt"))
                {
